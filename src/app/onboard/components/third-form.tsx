...
const ExperienceForm = () => {
  ...
  if (role === "talent") {
    fields = fields.filter(
      (field) =>
        !["hiring_for", "company_industry", "roles_looking_for"].includes(
          field,
        ),
    );
  }
  ...
  // check for companyyIndustry
  const hasMissingRequiredRecruiterFields =
    !companyIndustry || !rolesLookingFor.length;
  const hasMissingRequiredTalentFields = !skills.length;
  ...
  if (role === "recruiter") {
    form.append(
      "roles_looking_for",
      JSON.stringify(watch("data.roles_looking_for")),
    );
  }
  if (role === "recruiter") {
      ...
      <div className="flex flex-col gap-[36px] mt-[36px] text-[14px] font-normal">
        ...
        <div className="flex justify-between gap-3 h-[42px] w-full">
          ...
          <ButtonWithLoader
            isLoading={isPending}
            disabled={
              !isStepValid || !isValid || hasMissingRequiredFields || isPending
            }
...
const ChooseExperienceLevel = ({ level }: { level: string }) => {
  ...
  return (
    <Button
      onClick={() =>
        setValue("data.experience_level", level, { shouldDirty: true })
      }
      variant={"outline"}
      className={`cursor-pointer h-[42px] w-full flex-1 capitalize font-normal ${experience_level === level && "border-px border-primary"}`}
    >
...
const ChooseHirer = ({ hirer }: { hirer: string }) => {
  ...
  return (
    <Button
      onClick={() => setValue("data.hiring_for", hirer, { shouldDirty: true })}
      variant={"outline"}
      className={`cursor-pointer h-[42px] w-full flex-1 capitalize font-normal ${hiring_for === hirer && "border-px border-primary"}`}
    >
      {hirer}
    </Button>
  );
